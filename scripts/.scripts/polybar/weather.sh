...
APIKEY=`cat $HOME/secrets/owm-key`
...
function setIcons {
    ...
    if [ "$DISPLAY_WIND" = "yes" ] && [ `echo "$WINDFORCE >= $MIN_WIND" |bc -l` -eq 1 ]; then
        WIND="%{T$WEATHER_FONT_CODE}%{F$COLOR_WIND}$WINDICON"
...
#        TEMP="%{F$COLOR_COLD}%{T$TEMP_FONT_CODE}%{T-}%{F-} $TEMP_TEXT_BEGIN$TEMP%{T$TEMP_FONT_CODE}$TEMP_ICON%{T-}$TEMP_TEXT_END"
        TEMP="%{F$COLOR_COLD}%{T$TEMP_FONT_CODE}%{T-} $TEMP%{T$TEMP_FONT_CODE}$TEMP_ICON%{T-}%{F-}"
    elif [ `echo "$TEMP >= $HOT_TEMP" | bc` -eq 1 ]; then
        TEMP="%{F$COLOR_HOT}%{T$TEMP_FONT_CODE}%{T-} $TEMP%{T$TEMP_FONT_CODE}$TEMP_ICON%{T-}%{F-}"
    else
        TEMP="%{F$COLOR_NORMAL_TEMP}%{T$TEMP_FONT_CODE}%{T-} $TEMP%{T$TEMP_FONT_CODE}$TEMP_ICON%{T-}%{F-}"
    fi
}

function outputCompact {
    OUTPUT="$WIND %{T$WEATHER_FONT_CODE}%{F$ICON_COLOR}$ICON%{T-} $ERR_MSG$DESCRIPTION%{F-} $TEMP"
